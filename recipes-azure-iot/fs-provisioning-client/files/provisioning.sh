...
check_parameters() {
    if [ -z $prepare ]; then
        usage
    elif [ -z $prov ] || [ -z $id_scope ] || [ -z $iot_hub ] || [ -z $device_id ] || [ $DEVICE_PATH == "devices" ]; then
        usage
        exit 1
    fi
}
...
prepare_dir_for_customer() {
    ...
    if [ ! -f $CUSTOMER_PATH/certGen.sh ]; then
        cp $HOME_DIR/$CRT_PATH/certGen.sh $CUSTOMER_PATH
    fi

    if [ ! -f $CUSTOMER_PATH/openssl_device_intermediate_ca.cnf ]; then
        cp $HOME_DIR/$CRT_PATH/openssl_device_intermediate_ca.cnf $CUSTOMER_PATH
    fi

    if [ ! -f $CUSTOMER_PATH/openssl_root_ca.cnf ]; then
...
# CREATE DEST-DIR
create_dest_dir() {
    if [ -d $CUSTOMER_PATH/$DEVICE_PATH ]; then
...
create_dev_id() {
    if [ -f $CUSTOMER_PATH/$DEVICE_PATH/$device_id.txt ]; then
...
create_dev_certs() {
    ...
    if [ -d $CUSTOMER_PATH/$DEVICE_PATH/x509_c ]; then
        if [ -f $CUSTOMER_PATH/$DEVICE_PATH/x509_c/$device_id.cert.pem ] &&
            [ -f $CUSTOMER_PATH/$DEVICE_PATH/x509_c/$device_id.cert.pem ]; then
...
move_dev_certs_to_dest_dir() {
    for DEVICE_PATH in $DEVICE_PATH; do
...
delete_dev_certs() {
    if [ -f $CUSTOMER_PATH/certs/$device_id.cert.pem ]; then
        rm -f $CUSTOMER_PATH/certs/$device_id.cert.pem
    fi

    if [ -f $CUSTOMER_PATH/certs/$device_id.cert.pfx ]; then
        rm $CUSTOMER_PATH/certs/$device_id.cert.pfx
    fi

    if [ -f $CUSTOMER_PATH/private/$device_id.key.pem ]; then
        rm -f $CUSTOMER_PATH/private/$device_id.key.pem
    fi

    if [ -f $CUSTOMER_PATH/csr/$device_id.csr.pem ]; then
...
du_conf() {
    ...
    # Exported by fsup-provisioning.bbclass; fall back if script is run
    # standalone so it matches the runtime tmpfs mount point.
    local x509_store_path="${ADUC_X509_DIR:-/etc/adu/x509_c}"
    ...
    sed -i "s|<x509_store>|\"${x509_store_path}\"|g" "$config_file"
...
main() {
    create_dest_dir
    create_dev_id
    if [ -f $CUSTOMER_PATH/certs/$device_id.cert.pem ] && [ -f $CUSTOMER_PATH/private/$device_id.key.pem ]; then
...
if [ -n "$help" ]; then
    ...
    prepare_dir_for_customer
    create_ca
elif [ -n "$prov" ] && [ -n "$id_scope" ] && [ -n "$iot_hub" ] && [ -n "$device_id" ]; then
